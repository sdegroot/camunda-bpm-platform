    ...
    try {
      ...
      assertThat(e.getValidationReport())
          .hasVariableFailures("foo",
              "Cannot set variable of type object with name \"foo\". Java serialization format is forbidden");
    }
  }
    ...
    try {
      ...
      assertThat(e.getValidationReport())
          ...
          .hasVariableFailures("bar",
              "Cannot set variable of type object with name \"bar\". Java serialization format is forbidden");
    }
  }

  @Test
  public void shouldThrowValidationExceptionDueToInstructionsAndSerializationFormatForbiddenForVariable() {
    ProcessDefinition sourceProcessDefinition = testHelper.deployAndGetDefinition(ProcessModels.ONE_TASK_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper.deployAndGetDefinition(ProcessModels.ONE_TASK_PROCESS);
    ...
    try {
      ...
      assertThat(e.getValidationReport())
          .hasVariableFailures("foo",
              "Cannot set variable of type object with name \"foo\". Java serialization format is forbidden")
          .hasInstructionFailures("foo", "Source activity 'foo' does not exist", "Target activity 'bar' does not exist")
          .hasInstructionFailures("bar", "Source activity 'bar' does not exist", "Target activity 'foo' does not exist");
    }
  }
